...
const dataQa = qaTestData.INVALID_DATA.INVALID_LOGIN;
...
test('Username Benar Password Lorem Ipsum', async ({ page, browserName }, testInfo) => {
  //baris ini berfungsi untuk menginputkan data scenario id yang ada di test case
  ReportingApi.setTestCaseId('TC-UI-LOGIN-010');
  ...
  await page.getByRole('textbox', { name: 'Password' }).fill(dataQa.password);
  await page.getByRole('button', { name: 'Login' }).click();
  await expect(page.getByRole('alert')).toContainText(/failed|locked/i);
