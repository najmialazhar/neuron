...
test('Username Salah Password Salah', async ({ page, browserName }, testInfo) => {
  //baris ini berfungsi untuk menginputkan data scenario id yang ada di test case
  ReportingApi.setTestCaseId('TC-UI-LOGIN-003');
  ...
  await page.getByRole('textbox', { name: 'User code or email' }).fill(dataDev.username);
  await page.getByRole('textbox', { name: 'Password' }).fill(dataDev.password);
  await page.getByRole('button', { name: 'Login' }).click();
  await expect(page.getByRole('alert')).toContainText('user_auth_locked');
